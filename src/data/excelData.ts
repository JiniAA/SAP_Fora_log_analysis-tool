...
export async function loadExcelData(url = '/data/Logs_for_Analysis_1.xlsx'): Promise<DashboardData> {
  ...
  const execSheet = findSheet('executed tcodes') as Record<string, unknown>[];  // ← added
  ...
  const users: SAPUser[] = rawUsers.map(u => {
    ...
    // Derived flags — drives all status decisions cleanly
    const isLocked  = lockReason !== '' && lockReason !== '0';
    const isExpired = validToDate !== null && validToDate < today;

    // Status priority:
    //  1. Locked + valid date in future  → Locked but Valid
    //     (any lock reason including 'Administrator' with a future validTo)
    //  2. Locked + expired               → Inactive
    //     (lock reason present AND validTo is in the past)
    //  3. Not locked + expired           → Expired not Locked
    //     (validTo passed, no lock reason)
    //  4. Not locked + no logon > 90d   → Dormant
    //  5. Default                        → Active
  ...
  // ── 1. Descriptions from "executed tcodes" sheet (primary source) ──
  // Columns: TCode / T Code / TCODE  →  Transaction Text / Transaction text
  const execDescMap: Record<string, string> = {};
  execSheet.forEach(row => {
    const tc = String(
      row['TCode'] || row['T Code'] || row['TCODE'] || row['tcode'] || row['Tcode'] || ''
    ).trim();
    const desc = String(
      row['Transaction Text'] || row['Transaction text'] || row['transaction text'] || row['TransactionText'] || ''
    ).trim();
    if (tc && desc) {
      execDescMap[tc] = desc;
    }
  ...
  // ── 2. Transaction log tcodes set, counts & descriptions (fallback source) ──
  ...
  // Description priority per tcode:
  //   1. execDescMap[tc]    — from "executed tcodes" sheet  (most reliable)
  //   2. txTCodeDescMap[tc] — from "transactionlogs" sheet  (fallback)
  //   3. tc itself          — last resort if no description found anywhere
  const tCodes: SAPTCode[] = Array.from(allTCodesFromExcel).map(tc => ({
    tCode: tc,
    description: execDescMap[tc] || txTCodeDescMap[tc] || tc,
